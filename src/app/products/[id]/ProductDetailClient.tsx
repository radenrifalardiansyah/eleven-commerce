        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
              <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-50 transition-colors">
                <Minus className="w-4 h-4" />
              </button>
              <span className="w-12 text-center text-sm font-semibold text-gray-800">{qty}</span>
              <button onClick={() => setQty((q) => Math.min(product.stock, q + 1))} className="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-50 transition-colors">
          ...
          <div className="flex gap-3">
            <button disabled={!canBuy} className={cn('flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm transition-colors', canBuy ? 'bg-orange-100 text-primary-600 hover:bg-orange-200 border border-primary-300' : 'bg-gray-100 text-gray-400 cursor-not-allowed')}>
              <ShoppingCart className="w-5 h-5" />Keranjang
            </button>
            <button disabled={!canBuy} className={cn('flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm transition-colors', canBuy ? 'bg-primary-500 hover:bg-primary-600 text-white shadow-md shadow-primary-200' : 'bg-gray-300 text-gray-400 cursor-not-allowed')}>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setWishlisted((w) => !w)} className={cn('flex items-center gap-1.5 text-sm px-3 py-2 rounded-lg border transition-colors', wishlisted ? 'border-red-300 bg-red-50 text-red-600' : 'border-gray-200 text-gray-500 hover:border-gray-300')}>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                {product.seller.isVerified && <span className="text-xs bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded font-medium">✓ Verified</span>}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <h3 className="font-semibold text-sm text-gray-800 mb-3 flex items-center gap-2">
            <Package className="w-4 h-4 text-primary-500" />Info Pengiriman
          </h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between text-gray-600"><span>Dikirim dari</span><span className="font-medium text-gray-800">{product.location}</span></div>
            <div className="flex justify-between text-gray-600"><span>Berat produk</span><span className="font-medium text-gray-800">{product.weight}g</span></div>
            {[
              ...
              <div key={s.courier} className="flex items-center justify-between py-2 border-t border-gray-50">
                ...
                <div className="text-right"><span className="text-primary-600 font-semibold">{s.price}</span><span className="text-gray-400 text-xs ml-2">{s.eta}</span></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-10">
        <div className="flex border-b border-gray-100">
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key as 'desc' | 'reviews')} className={cn('flex-1 sm:flex-none px-6 py-4 text-sm font-semibold transition-colors border-b-2', activeTab === tab.key ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700')}>
              {tab.label}
            </button>
          ))}
        </div>
